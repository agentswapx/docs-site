...
- **OpenClaw** — 用 `clawhub` 或 `openclaw` 从 ClawHub 技能库安装已发布包，安装目录通常由 CLI 管理（如 `~/.openclaw/workspace/skills/atxswap/`），适合想少动目录、直接给智能体加能力。详见 [在 OpenClaw 中安装 ATX 技能](/zh/skill/install-openclaw)。
