...
ATXSwap 是 **BSC**（BNB Smart Chain）上的**智能体去中心化交换协议**，**主要面向 AI 智能体**：智能体通过 `atxswap` 技能与 TypeScript SDK 即可查价、兑换、管理流动性并完成链上转账；用户也可使用网站或钱包完成同类操作。
...
在 **OpenClaw**、**OpenCode**、**Claude Code** 等支持 [Agent 技能](https://github.com/anthropics/skills) 的环境里，可安装 `atxswap` 技能，用自然语言完成查价、兑换、流动性与转账。常见先选「运行时一键安装」或「本机/项目里放技能包」其中一种即可：
...
- **OpenCode** — 将技能放在项目的 `.opencode/skills/atxswap/` 或本机 `~/.config/opencode/skills/atxswap/`，进入目录执行 `npm install`；发现规则以 OpenCode 官方 [Agent Skills](https://opencode.ai/docs/skills) 为准。详见 [在 OpenCode 中安装 ATX 技能](/zh/skill/install-opencode)。
